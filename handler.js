...
export async function handler(chatUpdate) {
    ...
    global.img = 'https://telegra.ph/file/16770368c41f32cf80883.jpg' 
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            (chat.sBye || this.bye || conn.bye || 'Bye @user Kalok Balik Lagi Jan Lupa Bawak Gorengan')).replace(/@user/g, '@' + user.split`@`[0])
...
let msg = {
  ...
  if (msg) return conn.sendButtonDocAccess(m.chat, ucapan() + tag + '\n' + msg, wm, 'MENU', '.menu', m, adReply)
    ...
    let msgg = {
    	unreg: 'ʜᴀʟʟᴏ ᴋᴀᴋ 👋\n𝙰𝙽𝙳𝙰 𝙱𝙴𝙻𝚄𝙼 𝙼𝙴𝙽𝙳𝙰𝙵𝚃𝙰𝚁 𝚂𝙸𝙻𝙰𝙷𝙺𝙰𝙽 𝙼𝙴𝙽𝙳𝙰𝙵𝚃𝙰𝚁 𝙳𝙸 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴 𝙺𝙰𝙼𝙸 𝚄𝙽𝚃𝚄𝙺 𝙼𝙴𝙽𝙶𝙶𝚄𝙽𝙰𝙺𝙰𝙽 𝙵𝙸𝚃𝚄𝚁 𝙸𝙽𝙸\n\n➩𝙺𝙻𝙸𝙺 𝚃𝙾𝙼𝙱𝙾𝙻 𝙳𝙸 𝙱𝙰𝚆𝙰𝙷 𝚄𝙽𝚃𝚄𝙺 𝙼𝙴𝙽𝙳𝙵𝙰𝚃𝙰𝚁 𝙺𝙴 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴 𝙱𝙾𝚃'
}[type]
